...
(function(){
  ...
  function buildFullCatalogue(courses){
    var container = document.getElementById('catalogueDirectory');
    if (!container) return;
    ...
    for (var i = 0; i < courses.length; i++) {
      ...
      var key = (course.programme_code || '') + '|' + course.programme_name;
      if (!programmes[key]) {
        programmes[key] = {
          code: course.programme_code,
          name: course.programme_name,
          courses: []
        };
        order.push(key);
      }
      programmes[key].courses.push(course);
    }

    container.innerHTML =
      '<div class="catalogue-loading">Preparing full catalogue… ' +
      '<span id="catalogueProgress">0</span> / ' + order.length + ' programmes</div>';

    var progress = document.getElementById('catalogueProgress');
    var index = 0;
    var batchSize = 8;

    function renderBatch(){
      var fragment = document.createDocumentFragment();
      var stop = Math.min(index + batchSize, order.length);

      for (; index < stop; index++) {
        var programme = programmes[order[index]];
        var section = document.createElement('section');
        section.className = 'programme-block';

        var html = '<div class="programme-headline"><h3>' +
          escapeHtml(programme.name) +
          (programme.code ? ' (' + escapeHtml(programme.code) + ')' : '') +
          '</h3><span>' + programme.courses.length + ' courses</span></div>';

        html += '<table class="catalogue-table"><thead><tr>' +
          '<th>Course Code</th><th>Course Title</th><th>Year / Semester</th>' +
          '</tr></thead><tbody>';

        var lastSection = '';
        for (var c = 0; c < programme.courses.length; c++) {
          var item = programme.courses[c];

          if (item.section !== lastSection) {
            html += '<tr class="catalogue-section-label"><td colspan="3">' +
              escapeHtml(item.section || 'All Courses') + '</td></tr>';
            lastSection = item.section;
          }

          html += '<tr>' +
            '<td class="catalogue-code">' + escapeHtml(item.course_code) + '</td>' +
            '<td class="catalogue-course-title">' + escapeHtml(item.course_title) + '</td>' +
            '<td>' + escapeHtml(item.section || 'All Courses') + '</td>' +
            '</tr>';
        }
      ...
      if (progress) progress.textContent = index;
  ...
  function setupCatalogueToggle(){
    var button = document.getElementById('toggleCatalogue');
    var directory = document.getElementById('catalogueDirectory');
    if (!button || !directory) return;

    button.addEventListener('click', function(){
      var hidden = directory.classList.toggle('hidden');
      button.textContent = hidden ? 'Show Full Catalogue' : 'Hide Full Catalogue';
  ...
  function loadCatalogue(){
    ...
    fetch('courses.json?v=3', {cache:'no-store'})
      ...
      .then(function(payload){
        ...
        state.ready = true;
        buildFullCatalogue(courses);
        setupCatalogueToggle();
      ...
      .catch(function(){
        ...
        errorBanner.innerHTML =
          'The course database could not load. Confirm that <b>courses.json</b>, <b>app.js</b> and <b>index.html</b> are all uploaded in the repository root, then hard-refresh the page.';
  ...
  window.setTimeout(function(){
    var directory = document.getElementById('catalogueDirectory');
    if (directory && directory.getAttribute('data-loaded') !== 'true') {
      var loadingText = directory.querySelector('.catalogue-loading');
      if (loadingText) {
        loadingText.innerHTML =
          'Still loading the catalogue. Please refresh once. If this remains, verify that <b>courses.json</b> and <b>app.js</b> were replaced on GitHub.';
      }
    }
  }, 12000);
